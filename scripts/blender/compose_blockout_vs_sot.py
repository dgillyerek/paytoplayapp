...
def fit(im: Image.Image, box: tuple[int, int], bg=(232, 232, 230)) -> Image.Image:
    im = im.convert("RGB")
    # drop near-white / near-grey margins
...
def main() -> None:
    ...
    cw, ch = 420, 780
    sheet = Image.new("RGB", (cw * 2, ch * len(pairs) + 110), (24, 24, 22))
    d = ImageDraw.Draw(sheet)
    title = font(26)
    lab = font(18)
    d.text((24, 24), "GATE 2 BLOCKOUT  ·  vs  GATE 1 TURNAROUND", fill=(236, 230, 210), font=title)
    d.text((24, 58), "Silhouette / proportions only  ·  look + motion NOT claimed  ·  grey clay", fill=(210, 176, 82), font=lab)
    ...
    dest = ART / "aldric_gate2_blockout_vs_turnaround.png"
    sheet.save(dest)
    for name in ("blockout_front.png", "blockout_back.png", "blockout_side_r.png", "blockout_three_quarter.png"):
        (ART / f"aldric_gate2_{name}").write_bytes((PROOF / name).read_bytes())
