#!/usr/bin/env python3
"""Sir Aldric SEP mid450k look Play-cam remap (no Unity Editor on this VM).

LOOK DENSITY SWAP ONLY. Walk AccuRIG + Walking clip unchanged.
UV-stamps sep_paint atlas (same UV) onto that AccuRIG. mid450k is look SoT
UV source. mid80k sword+scabbard parents at character-RIGHT hip (offsets
unchanged). No Path A. No attack rewire. No LEFT hip.
...
PROOF = ROOT / "Docs/Survival/previews/aldric_sep_paint_mid450k_20260926"
...
def main():
    lo = os.environ.get("ALD_MID450_PROBE", "0") == "1"
    ...
    print("look body", body.name, "faces", len(body.data.polygons), "verts", len(body.data.vertices))
    ...
    print("look sword", sword.name, "faces", len(sword.data.polygons), "verts", len(sword.data.vertices))
    ...
    look(cam, *CAM_REAR)
    render_to(PROOF / "sir_aldric_sep_paint_mid450k_rear_walk_playcam.png")
    look(cam, *CAM_FRONT)
    render_to(PROOF / "sir_aldric_sep_paint_mid450k_front_walk_playcam.png")
    look(cam, *CAM_34)
    render_to(PROOF / "sir_aldric_sep_paint_mid450k_34_walk_playcam.png")

    if lo:
        print("probe stills only")
        return

    frames_dir = PROOF / "_paint_mid450k_walk_frames"
    ...
    mp4 = PROOF / "sir_aldric_sep_paint_mid450k_walk_juice_playcam.mp4"
