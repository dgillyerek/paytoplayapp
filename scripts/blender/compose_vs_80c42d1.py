#!/usr/bin/env python3
"""Side-by-side 80c42d1 FAIL MP4 frames vs blended-loft / steel-texel iterate."""
...
def main() -> None:
    ...
    if not cells:
        raise SystemExit("no vs-80c42d1 pairs")
    w, h = cells[0][0].size
    sheet = Image.new("RGB", (w * 2, 110 + h * len(cells)), (16, 16, 14))
    d = ImageDraw.Draw(sheet)
    d.text(
        (16, 12),
        "GATE 3  |  80c42d1 FAIL tear MP4  vs  this walk MP4  |  bind NOT claimed",
        fill=(236, 230, 210),
        font=font(22),
    )
    d.text(
        (16, 48),
        "LEFT = 80c42d1 (one loft, horizontal tear).  RIGHT = steel texel + blended loft weights.  e5b132f.",
        fill=(180, 176, 160),
        font=font(15),
    )
    d.text(
        (16, 74),
        "Both columns are MP4 frames. One arm/side must hold. Eye tear stretch at n=0 and n=15.",
        fill=(180, 176, 160),
        font=font(15),
    )
    ...
    out = PROOF / "gate3_path2_vs_80c42d1.png"
    sheet.save(out, optimize=True)
    (ART / out.name).write_bytes(out.read_bytes())
    print("vs-80c42d1", out, out.stat().st_size)
