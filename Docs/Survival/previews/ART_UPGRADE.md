...
- **Motion now:** 4 Game-view keys (`pass L / contact L / pass R / contact R`) solved against `WALK_GAIT_BAR` rear phases on this hang −Y rig. This camera foreshortens sagittal flex — 70° still parked both feet on one screen row. Pass thigh **+24°** / knee **78°** lifts ~0.36 m so the passing foot reads above the plant; contact lead is only −18° so the plant stays visible below the hips. Contralateral pendulum ±32–36° X with Z ±22 abduction, hip drop `Hips.Z=±14`, spine yaw opposite the pelvis, heel→toe foot X. Attack clip unchanged. March +Z = TOP, 0.80 m/s.
